...
def modem(binary_vector, modulation_type="QAM", M=16, code_label="Binary"):
    ...
    # Modulation
    symbols, symbol_idxs = modulate_symbols(binary_vector, modulation_type, M, code_label)
    print(f"\nConstellation points for {modulation_type} modulation with M={M} and {code_label} code:", symbols, sep="\n")

    # Generate constellation plot
    plot_constellation(modulation_type, symbols, M, MEDIA_PATH, filename=f"{modulation_type}_{M}_{code_label}_constellation.png")

    # Calculate mean energies
    Eb_theo = 1
    Es_theo = Eb_theo * np.log2(M)

    Es_mean, Eb_mean = calculate_mean_energies(symbols, M)
